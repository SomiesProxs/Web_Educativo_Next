...
export async function POST(request: NextRequest) {
  try {
    const { nombreNivel } = await request.json();

    if (!nombreNivel || typeof nombreNivel !== 'string') {
      return NextResponse.json({ error: 'Nombre de nivel inválido' }, { status: 400 });
    }

    const nombreMayus = nombreNivel.toUpperCase();
    const nombreCapitalizado = nombreNivel.charAt(0).toUpperCase() + nombreNivel.slice(1).toLowerCase();
    const componenteNombre = `${nombreMayus}${nombreCapitalizado}`;

    const carpetaBase = path.join(process.cwd(), 'app', 'ADMIN', 'ComponenteAdmin', 'Cursos', nombreMayus);
    const archivoPath = path.join(carpetaBase, `${nombreCapitalizado}.tsx`);

    // Verificar si ya existe
    try {
      await fs.access(archivoPath);
      return NextResponse.json({ error: 'El nivel ya existe.' }, { status: 409 });
    } catch {
      // Continúa si no existe
    }
...
interface ${componenteNombre}Props {
  onVolver: () => void;
}

export default function ${componenteNombre}({ onVolver }: ${componenteNombre}Props) {
  ...
  return (
    <div className="p-8 rounded-lg max-w-3xl mx-auto">
      ...
      <h2 className="text-3xl font-bold mb-6">TEMAS Y SUBTEMAS DE ${nombreMayus}</h2>
...
`;
    ...
    return NextResponse.json({ success: true, nivel: nombreMayus });
  } catch (error) {
    console.error(error);
    return NextResponse.json({ error: 'Error al crear el nivel' }, { status: 500 });
  }
}